 ...
 * @author lokesh
 */
require_once(realpath($_SERVER["DOCUMENT_ROOT"]) . '\my-project\src\Model\Database.php');

class Database_class extends Database {

    public $connection = Database::get_connection;
...
// Inserting in Table(time Information) of Database
    ...
    public function get_max__model_in_month($day) {
        ...
        $data1 = $instance_of_class->get_max_query($data);
        return $data1;
    }
    ...
    public function get_max_model_in_week($date) {
        ...
        $data1 = $instance_of_class->get_max_query($data); //get  model that has maximum enquaries
        return $data1;
    }

}

function get_max_query($data) { //function returns the model that has maximum enquaries
